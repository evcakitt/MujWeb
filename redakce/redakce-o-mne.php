...
<head>
  <meta charset="utf-8">
  <title>Administrtivní zóna Kateřiny Novotné - O mně</title>
...
<body>
  ...
  <div class="sectionr">
    ...
    if (isset($_POST['editor1'])) {
      ...
      if ($chyba == FALSE) {
        ...
        if ($data = mysqli_query($spojeni, $dotaz)) {
          ...
          if ($radek != NULL) {
            $hlaska_duplicita = '<div class="komentar_vlozeni">Text stránky byl uložen.</div>';
            $exist = TRUE;
          } else {
    ...
    /*kontrola zda je klient přihlasen a pak zobrazeni stranky*/
    if (empty($volej)) {
      $volej = "nic";
    }
    if ($volej != "ano") {
      ...
      if ($chyba1 == FALSE) {
        ...
        if ($exist1 == FALSE) {
          ...
          if (mysqli_query($spojeni, $dotaz1)) {
            ...
            $hlaska1 = '<div class="komentar_vlozeni">Popis stranky se nepodařilo vlozit.</div>';
          }
        }
        /* Kontrola, zda zaznam jiz v DB existuje */
        $exist2 = FALSE;
        ...
        if ($data2 = mysqli_query($spojeni, $dotaz2)) {
          ...
          if ($radek2 != NULL) {
            $hlaska_popis = '<div class="komentar_vlozeni">N�zev byl ulo�en.</div>';
        ...
        if ($exist2 == FALSE) {
          ...
          if (mysqli_query($spojeni, $dotaz2)) {
            $hlaska1 = '<div class="komentar_vlozeni">N�zev byl upraven.</div>';
          } else {
            $hlaska1 = '<div class="komentar_vlozeni">N�zev se nepoda�ilo vlo�it.</div>';
          }
        }
    ...
    ?>
    <?php
    if ($volej != "ano") {; //musite se prihlasit
    } else {
      require "title_a_popis.php";
    }
    ?>
    <!-- KONEC M\iSTA PRO VKLADANI -->
    <!-- konec section -->
  </div>
  </div>
